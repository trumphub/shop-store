...
export default {
  ...
  methods: {
    async getGoodsById() {
      const { data } = await Api.getGoodsById({ goodsId: this.goodsId });
      this.item.goods_name = data.goodsInfo.goods_name;
      this.item.categoryIds = data.goodsInfo.categoryIds;
      this.item.imagesIds = data.goodsInfo.goods_images.map(
        (img) => img.file_id
      );
      this.fileList1 = data.goodsInfo.goods_images.map((img) => ({
        name: img.file_name,
        url: img.external_url,
        id: img.file_id,
      }));
      this.item.goods_no = data.goodsInfo.goods_no;
      this.item.delivery_id = data.goodsInfo.delivery_id;
      this.item.status = data.goodsInfo.status;
      this.item.sort = data.goodsInfo.sort;
      this.item.spec_type = data.goodsInfo.spec_type;
      if (this.item.spec_type === 10) {
        this.item.goods_price = data.goodsInfo.skuList[0].goods_price;
        this.item.line_price = data.goodsInfo.skuList[0].line_price;
        this.item.stock_num = data.goodsInfo.skuList[0].stock_num;
        this.item.goods_weight = data.goodsInfo.skuList[0].goods_weight;
      } else {
        this.item.specData.specList = data.goodsInfo.specList;
        this.specItemKey = this.item.specData.specList.length;
        for (let j = 0; j < this.item.specData.specList.length; j++) {
          const element = this.item.specData.specList[j];
          this.specItemValKey.push(element.valueList.length);
        }
        const tmp = data.goodsInfo.skuList.map((sku) => {
          return {
            ...
            goods_sku_no: sku.goods_sku_no,
            image: sku.image,
          };
        });
        this.tableData.forEach((t, idx) => {
          t.goods_price = tmp[idx].goods_price;
          t.goods_sku_no = tmp[idx].goods_sku_no;
          t.goods_weight = tmp[idx].goods_weight;
          t.image_id = tmp[idx].image_id;
          t.line_price = tmp[idx].line_price;
          t.stock_num = tmp[idx].stock_num;
          t.imageList = tmp[idx].image
            ? [
                {
                  name: tmp[idx].image.file_name,
                  url: tmp[idx].image.external_url,
                },
              ]
            : [];
        });
        this.item.specData.skuList = this.tableData;
      }
      this.item.deduct_stock_type = data.goodsInfo.deduct_stock_type;
      this.item.content = data.goodsInfo.content;
      if (data.goodsInfo.video_id !== 0) {
        this.item.video_id = data.goodsInfo.video_id;
        this.videoUrl = data.goodsInfo.video.external_url;
      }
      if (data.goodsInfo.video_cover_id !== 0) {
        this.item.video_cover_id = data.goodsInfo.video_cover_id;
        this.fileList2 = [
          {
            name: data.goodsInfo.videoCover.name,
            url: data.goodsInfo.videoCover.external_url,
          },
        ];
      }
      this.item.selling_point = data.goodsInfo.selling_point;
      this.item.serviceIds = data.goodsInfo.serviceList || [];
      this.item.sales_initial = data.goodsInfo.sales_initial;
      this.item.is_points_gift = data.goodsInfo.is_points_gift;
      this.item.is_points_discount = data.goodsInfo.is_points_discount;
      this.item.is_enable_grade = data.goodsInfo.is_enable_grade;
      this.item.is_alone_grade = data.goodsInfo.is_alone_grade;
      if (this.item.is_enable_grade === 1) {
        for (let i = 0; i < this.gradeList.length; i++) {
          const element = this.gradeList[i];
          this.$set(
            this.item.alone_grade_equity,
            `grade_id:${element.grade_id}`,
            data.goodsInfo.alone_grade_equity[element.grade_id]
          );
        }
      }
      this.loading = false;
    ...
    handleRemoveImage({ response, id }) {
      if (response) {
        ...
        this.item.imagesIds = this.item.imagesIds.filter((item) => item !== id);
      }
    },
    ...
    submitForm() {
      this.$refs.form.validate((v) => {
        if (v) {
          ...
          this.loading = true;
          Api.updateGoods({ form: good, goodsId: this.goodsId })
            .then(({ message }) => {
              this.loading = false;
              this.$message.success(message);
